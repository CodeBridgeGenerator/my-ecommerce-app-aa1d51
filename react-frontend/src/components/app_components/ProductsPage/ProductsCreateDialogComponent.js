...
const ProductsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            id: _entity?.id,name: _entity?.name,description: _entity?.description,price: _entity?.price,quantity: _entity?.quantity,imageURL: _entity?.imageURL,category: _entity?.category,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText className="w-full mb-3 p-inputtext-sm" value={_entity?.imageURL} onChange={(e) => setValByKey("imageURL", e.target.value)}  required  />
            </span>
            <small className="p-error">
